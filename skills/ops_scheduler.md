...
2. **Deadline Prioritization**: Higher priority should be assigned to tasks with closer deadlines that are lagging in status (e.g. still in "scripting" when due in 3 days).
3. **Freelancer Offloading**: Identify tasks in the "production" stage that can be safely outsourced to active freelancers to alleviate in-house bottlenecks.
...
```json
{
  ...
  "danger_tasks": [
    {
      "task_id": 12,
      "reason": "Due in 2 days but still in scripting status."
    }
  ],
  "reorder_plan": [
    {
