...
function buildWordmark() {
  ...
  m.font = '800 ' + SZ + 'px "Onest", system-ui, -apple-system, sans-serif';
  ...
  for (let i = 0; i < word.length; i++) {
    ...
    const asc = t.actualBoundingBoxAscent || SZ * 0.7;
    const desc = t.actualBoundingBoxDescent || 0;
    const l = t.actualBoundingBoxLeft || 0;
    const r = t.actualBoundingBoxRight || t.width;
    const g = { ch: ch, adv: t.width, asc: asc, desc: desc, l: l, r: r, pen: pen };
    gl.push(g);
    ascMax = Math.max(ascMax, g.asc);
    descMax = Math.max(descMax, g.desc);
    xMin = Math.min(xMin, pen - g.l);
    xMax = Math.max(xMax, pen + g.r);
    // Tight tracking after hyphen so it stays snug with I; balanced tracking between I, N, G
    const track = ch === '-' ? 0.08 * SZ : 0.22 * SZ;
    pen += t.width + track;
  }
  ...
  gl.forEach((g, i) => {
    if (g.ch === ' ') return;
    const cw = Math.max(4, Math.ceil(g.l + g.r) + PAD * 2);
    const chh = Math.max(4, Math.ceil(g.asc + g.desc) + PAD * 2);
    const c = cvs(cw, chh), x = c.getContext('2d');
    x.font = '800 ' + SZ + 'px "Onest", system-ui, -apple-system, sans-serif';
  ...
  scene.add(group);
  WORD.group = group;
  WORD.ink = { xMin: xMin, xMax: xMax, cx: (xMin + xMax) / 2, w: Math.max(1, xMax - xMin), asc: ascMax };
}
...
function buildCardCloth() {
  ...
  const frames = $$('.cards .card-fr, #cur .les-media');
  frames.forEach(fr => {
    let url = (getComputedStyle(fr).backgroundImage.match(/url\(["']?([^"')]+)/) || [])[1];
    if (!url) {
      const imgEl = fr.querySelector('img');
      if (imgEl) url = imgEl.currentSrc || imgEl.src;
    }
    if (!url) return;
    const isLes = fr.classList.contains('les-media');
    const img = new Image();
    img.onload = () => {
      const dpr = Math.min(devicePixelRatio || 1, 2);
      ...
      const card = fr.closest('.card') || fr.closest('.les') || fr;
      card.addEventListener('pointerenter', () => inst.setEdge(.185), { passive: true });
      card.addEventListener('pointerleave', () => {
        if (inst.leave) inst.leave();
        inst.setEdge(.048);
      }, { passive: true });
      if (inst.pointer) {
        card.addEventListener('pointermove', e => {
          const r = fr.getBoundingClientRect();
          inst.pointer(e.clientX - r.left, e.clientY - r.top);
        }, { passive: true });
      }
    };
    img.onerror = () => {};
...
function wireNav() {
  ...
  /* The page has no header of its own — the site's React Navbar is the only
     one — so everything below that drives a nav bar is wired only if one is
     actually present. The rail and section tracking further down always run. */
  if (nav && burger) {
    const closeMenu = () => {
...
function wirePartnerForm() {
  ...
  form.addEventListener('submit', function(e) {
    ...
    if (!name || !email || !brand || !message) {
      status.className = 'partner-form-status active';
      status.style.borderColor = '#e0231c';
      status.innerHTML = '<span style="color:#e0231c;">Please fill out all required fields marked with *.</span>';
      return;
    }

    const recipient = 'deepanjan@hypheningmedia.com';
    const subject = `[Hyphening Inquiry] ${brand} - ${name}`;
    const bodyLines = [
      'Hello Deepanjan,',
      '',
      'A new partnership inquiry was submitted from the Hyphening Media website:',
      '',
      '----------------------------------------',
      `• Name: ${name}`,
      `• Work Email: ${email}`,
      `• Brand / Company: ${brand}`,
      `• Area of Focus: ${service || 'Full-Spectrum Creative Operations'}`,
      '----------------------------------------',
      '',
      'Project Brief & Goals:',
      message,
      '',
      '----------------------------------------',
      'Sent via Hyphening Media Website (Trusted Partners)'
    ];
    ...
    status.className = 'partner-form-status active';
    status.innerHTML = `
      <div style="font-weight:600; color:#dfe7e0; margin-bottom:6px;">✓ Dispatching Inquiry to ${recipient}</div>
      <div style="color:rgba(223,231,224,0.7); font-size:12px;">Opening your email client to send to Deepanjan. If it did not open automatically, <a href="${mailtoUrl}">click here to email directly</a>.</div>
    `;
...
window.navigateToSection = function(targetId) {
  if (!targetId || targetId === 'top' || targetId === 'hero') {
  ...
  const t = document.getElementById(actualId) || document.querySelector('#' + actualId);
  if (t) {
    const topPos = actualId === 'top' ? 0 : Math.max(0, t.offsetTop - 50);
    window.scrollTo({ top: topPos, behavior: REDUCE ? 'auto' : 'smooth' });
  }
};
window.addEventListener('message', function(e) {
  if (e.data && e.data.type === 'NAVIGATE_TO_SECTION') {
    window.navigateToSection(e.data.targetId);
  }
});
...
function buildCards() {
  const defs = [
    ...
    { p: [-4.2, 2.80, -14.0], t: [-1.0, 4.5, -24.0], fov: 38 }, /* maple ridge   */
    { p: [3.2, 1.90, -5.0], t: [0, 7.5, -20.0], fov: 40 }       /* moon court     */
  ];
...
function render() {
  FRAME++;
  /* POST.scene only exists once initPost() has run. Resolve it once and reuse
     it, so a scene that somehow started early renders straight to the canvas
     instead of handing three.js an undefined render target. */
  const post = (WANT_POST && POST.scene) ? POST.scene : null;
  renderer.setRenderTarget(post);
  renderer.clear(true, true, false);
...
function frame(now) {
  ...
  applyCamera();
  updateWorld(dt);
  /* A throw inside render() used to skip queue() and kill the loop for good,
     leaving a black page. Survive a blip; give up gracefully on a real fault. */
  try { render(); renderFails = 0; }
  catch (err) {
    if (++renderFails === 1) console.error('[kage] render failed', err);
    if (renderFails > 3) { running = false; return fallback(err); }
  }
  queue();
}
...
const JOBS = [
  ['Reading the type', () => {
    ...
    return Promise.race([
      Promise.all([
        document.fonts.load('800 320px "Onest"').catch(() => {}),
...
function boot() {
  ...
  /* Watchdog, not a deadline. Every job below is synchronous except the font
     load, which caps itself at 250ms, so a stall means something is broken
     rather than merely slow — and a slow phone must be allowed to finish.
     It degrades to the static page: starting the renderer on a half-built
     scene leaves POST.scene undefined, which throws on frame 1 and paints
     the whole background black. */
  const watchdog = setInterval(() => {
    if (started) { clearInterval(watchdog); return; }
    /* Per job, not for the whole boot. The slowest job measured is ~2.6s on a
       slow machine, so 12s means a genuine hang, never a slow phone. */
    if (performance.now() - lastProgress > 12000) {
      clearInterval(watchdog);
      console.error('[kage] boot stalled on job "' + (JOBS[i] && JOBS[i][0]) + '"');
      fallback(new Error('boot stalled on job ' + i));
    }
  }, 1000);
  ...
  const step = () => {
    ...
    const done = () => {
      ...
      /* Every job has to finish before start(): the last one runs initPost(),
         which builds the render targets the loop dereferences immediately. */
      if (i < JOBS.length) setTimeout(step, 16);
      else { clearInterval(watchdog); setTimeout(start, 120); }
    };
    let r;
    try { r = j[1](); }
    catch (err) {
      console.error('[kage] job "' + j[0] + '" failed', err);
      if (i <= 1) { clearInterval(watchdog); return fallback(err); }
    }
...
function fallback(err) {
  started = true;                              /* boot is resolved — never start() after this */
