...
## Gated Execution & Dashboard Integration

Every agent execution triggered under the Hyphening Marketing Ops Center must conform to the following verification gates before executing subagents or querying live APIs:

1. **Pre-Execution Gate & Approvals**:
   - Check if this audit run has an approved record in the dashboard queue (`openclaw_pending_actions`).
   - If the task is triggered by an administrator, it is `auto_approved` and can execute immediately.
   - If triggered by an SMM or staff freelancer, it is `pending_approval`. **Refuse execution** and output a message directing them to request admin approval from the **Approvals Tab** in the dashboard.

2. **Staleness and Duplicate Run Guards**:
   - Prior to running, cross-reference the agent’s execution history for this client.
   - If a previous run of this agent exists, check the time elapsed:
     - If the age of the last audit is **less than** the configured `stale_after_days` for this agent type (seed config defaults: `technical` is 14 days, `backlinks` is 30 days, `local` is 7 days), the audit is considered **Fresh**.
     - **Warning Prompt**: Display a prominent warnings message: `"WARNING: This agent was run recently and is still fresh. Running again will consume API tokens. Do you wish to force execution?"`. Only proceed if explicitly forced by the admin.

3. **Monthly Budget Enforcement**:
   - Check the client's current accumulated monthly spend from `token_usage_log`.
   - Compare it against their limit in `token_budgets.monthly_budget_usd`.
   - If `hard_stop = 1` and the spend exceeds the limit: **Immediately block execution** and alert the user that the monthly token budget cap has been reached.
   - If the spend exceeds the `alert_threshold_pct` (e.g. 80%), output a warning banner but allow execution to continue.

4. **Model Selection Routing**:
   - **DeepSeek V4 Flash**: Use this model for highly structured, template-driven, and technical checks (technical audit crawls, sitemaps, structured schema detection, hreflang tags, image checks) to maintain low token budgets.
   - **Kimi**: Use this model for judgment-heavy, strategic, and quality checks (content E-E-A-T, drift history comparisons, AI Overview/GEO optimization, keyword clustering, and backlinks competitor pitch generation).

5. **Webhook Data Delivery**:
   - Upon completion, the agent MUST pack the results and POST them as an HMAC-signed payload to `/api/openclaw/webhook`.
   - Compute the signature using the SHA256 HMAC of the stringified payload and the `OPENCLAW_HMAC_SECRET`. Put it in the `x-openclaw-signature` header.
   - The JSON payload must follow this exact schema:
    ...
    "recommendations": [
      {
        ...
        "metric": "Category / Tag name",
        "issue": "Detailed description of the issue found",
        "action_required": "Step-by-step remediation action required"
      }
    ],
    "report_json": {
      "timestamp": "2026-07-17T09:35:10Z",
      "target": "https://example.com",
      "client": "Client Name",
      "competitor_gap": [
        {
          "site_name": "Competitor Site",
          "site_url": "https://competitor.com/blog",
          "category": "guest_post",
          "da": 45
        }
      ]
